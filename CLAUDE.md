...
`createOffer` now actually uploads the picked photo instead of leaving it local-only: if `payload.imageUri` is set, it's first passed through `compressImage` (resizes to fit `MAX_UPLOAD_DIMENSION` = 1280px on the long edge via `ImageManipulator.manipulate(uri).resize(...)`, re-encoded as JPEG at `UPLOAD_JPEG_COMPRESSION` = 0.75) and then `uploadImage` (multipart `POST /uploads`, auth required, returns `{url}`) before the offer itself is created with that URL as `imageUrl`.

`uploadImage` branches on `Platform.OS`, and the split is deliberate, not incidental — both JS-`fetch`-based approaches were tried first and both broke on real SDK 57 behavior, not just in theory:
- React Native's old `{uri, name, type}` FormData convention → Expo's own fetch/FormData runtime (`expo/src/winter/fetch/convertFormData.ts`, replacing RN's built-in as of SDK 53+) doesn't recognize it at all and throws `Unsupported FormDataPart implementation` at request time (not a compile error).
- Re-`fetch`ing the local file URI into a `Blob` and appending that to `FormData` → works fine on web (confirmed end-to-end), but on native goes through React Native's `Blob` polyfill (base64 round-trip through the native blob store — Expo even warns about this and suggests `expo-blob`) and the backend received corrupted/undecodable image bytes (400 "invalid image"), even though the request itself succeeded.

So: **native** uses `expo-file-system`'s `File.upload()` (`uploadType: UploadType.MULTIPART`) — a native-backed multipart upload that reads the file directly of disk, bypassing the JS fetch/FormData layer (and its `Blob` polyfill) entirely. **Web** keeps the `Blob`+`fetch`+`FormData` path, since a real browser's implementations are standards-compliant and this already works. If a third upload approach is ever needed, verify it against an actual device, not just `tsc`/web — every JS-level shortcut here has broken in a way that only showed up at runtime.
...
- `CreateOfferScreen`'s `pickImage` tries the camera first (`requestCameraPermissionsAsync` then `launchCameraAsync`) since a photo taken in the moment is the common case for an offer; it only falls back to `launchImageLibraryAsync` (gallery) when permission is refused or the camera itself throws (e.g. no camera hardware, like a simulator). A deliberate cancel out of an otherwise-working camera does **not** fall back to the gallery — that would be surprising, not helpful. `app.json`'s `expo-image-picker` plugin config sets the camera/photos permission strings (and disables the unused microphone permission).
